...
class DummyPlayerNode(Node):
    def __init__(self) -> None:
        super().__init__("dummy_player")
        self.declare_parameter("player_name", self.get_name())
        self.declare_parameter("plan_turn_service", "/dummy_player/plan_turn")
    ...
    def _compute_ik(
        ...
        """Call MoveIt /compute_ik to solve for the panda_arm group.

        ``target_pose`` is the desired pose of ``panda_link8`` (the group
        tip link).  The caller is responsible for applying the fixed TCP
        offset before calling this method.
        """
    ...
    def _link8_pose_from_tcp_target(
        ...
        """Build the panda_link8 target pose for a desired TCP position.

        Assumes gripper-down orientation and accounts for the fixed Z
        offset between link8 and panda_hand_tcp.
        """
        pose = Pose()
        pose.position.x = x
        pose.position.y = y
    ...
    def _handle_plan_turn(
        ...
        move = self._select_scripted_move(request)
        if move is None:
        ...
        # Resolve target positions from the request data.
    ...
    def _find_piece_pose(request: PlanTurn.Request, piece_id: int) -> Pose:
        for piece in request.snapshot.pieces:
            if piece.piece_id == piece_id:
                return piece.pose
        # Fallback to initial pieces in layout.
    ...
    @staticmethod
    def _select_scripted_move(request: PlanTurn.Request) -> tuple[int, int] | None:
        available = {
            piece.piece_id
            for piece in request.snapshot.pieces
        ...
        for piece_id, cell_id in SCRIPTED_MOVES:
            if piece_id in available and cell_id in legal:
